...
const Form = ({ children, className, onSubmit }: FormProps) => (
    <form onSubmit={onSubmit} className={className} children={children} />
);
...
const formSchema = z.object({
    ...
    framework: z
        .string()
        .refine((val) => frameworks.includes(val as any), {
    ...
    return (
        <div className="max-w-md mx-auto p-6 bg-white rounded-lg shadow-md">
            ...
            <Form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                <FormField label="Имя" name="firstName" control={control} errors={errors}>
                    {(field) => <Input {...field} />}
                </FormField>

                <FormField label="Фамилия" name="lastName" control={control} errors={errors}>
                    {(field) => <Input {...field} />}
                </FormField>

                <FormField label="Возраст" name="age" control={control} errors={errors}>
                    {(field) => (
                        <NumberInput
                            id="age"
                            {...field}
                ...
                <FormField label="Фреймворк" name="framework" control={control} errors={errors}>
                    {(field) => (
                        <select
                            {...field}
                            id="framework"
